...
def get_handler(key):
    ...
    return jsonify({"message": "Key requested successfully", "key": key, "value": int(value)}), 200
...
def set_handler(key):
    ...
    old_value = r.get(key)
    r.set(key, value)
    return jsonify({"message": "Key set successfully", "key": key, "value": int(value), "old_value": int(old_value)}), 200

@app.route('/api/v1/hit/<key>', methods=['GET'])
def hit_handler(key):
    new_value = r.increase(key)
    return jsonify({"message": "Key updated successfully", "key": key, "value": int(new_value)}), 200
